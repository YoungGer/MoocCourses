              ...
              window.imagePicker.getPictures(
                  function(results) {
                      for (var i = 0; i < results.length; i++) {
                          ...
                          // $scope.collection.imgSrc[i] = "data:image/jpeg;base64," + results[i];
                          $scope.selection.push({'imgSrc':results[i]})
                      }
...
.controller('MenuController', ['$scope', 'dishes', 'favoriteFactory', 'baseURL', '$ionicListDelegate', '$ionicPlatform', '$cordovaLocalNotification', '$cordovaToast','$localStorage', function ($scope, dishes, favoriteFactory, baseURL, $ionicListDelegate, $ionicPlatform, $cordovaLocalNotification, $cordovaToast,$localStorage) {
        ...
        .controller('DishDetailController', ['$scope', '$stateParams', 'dish', 'menuFactory', 'favoriteFactory', 'baseURL', '$ionicPopover', '$ionicModal', '$ionicPlatform', '$cordovaLocalNotification', '$cordovaToast',function ($scope, $stateParams, dish, menuFactory, favoriteFactory, baseURL, $ionicPopover, $ionicModal,$ionicPlatform, $cordovaLocalNotification, $cordovaToast) {
            ...
            $scope.doReserve = function() {
              ...
              $scope.dish.comments.push({
                'rating':$scope.popdata.star,
                'comment':$scope.popdata.name,
                'author': $scope.popdata.comment,
                'date': new Date()
              });
              $timeout(function() {
                $scope.closeReserve();
        ...
        // implement the IndexController and About Controller her
        ...
        .controller('AboutController', ['$scope', 'corporateFactory','leaderFactory','baseURL','leaders','leadership',  function($scope, corporateFactory,leaderFactory,leaders,leadership) {
            
                    // $scope.leaders = corporateFactory.query();
                    $scope.leaders = leaders;
                    console.log($scope.leaders);
                    // $scope.leadership = leaderFactory.query();
